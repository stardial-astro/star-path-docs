...
::: callout info "关于速度"
作为一款非营利的学术用途的应用，我们的服务器搭载使用的是免费计划，因此连接和运算时间偶尔会比往常长一些。但通常最多等待几秒钟即可获得结果。
:::
...
## 图例和数据表格 {#legend-and-table}

图中的星轨根据不同晨昏阶段使用不同线型绘制加以区分。所选天体在升落和中天的位置也标记在图中。图片下方附带了图例和数据表格供参考。表格中包含了这些标记位置的相应坐标和时刻。
