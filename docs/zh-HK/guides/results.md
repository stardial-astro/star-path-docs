...
天體的視運動軌跡投影在[地平座標系][horizontal coordinate system]上。該座標系的兩個獨立座標分別為**高度角**（alt.）和**方位角**（az.）。圖中最外圈表示觀測位置的地平線。[南北天極][north and south celestial poles]和[天頂][zenith]如果在座標範圍內，將分別標記為 `NCP`、`SCP` 和 `Z`。
